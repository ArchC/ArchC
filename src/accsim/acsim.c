...
/*  {"--inline"        , "-i"          ,"Inline ISA and Syscalls (slow compilation, fast execution)", "r"},*/
  ...
  {"--annul-instr"   , "-ai"         ,"Necessary in models which the instructions may be executed or not", "r"},
...
void CreateResourceHeader() {
  ...
  //ILP method
  COMMENT(INDENT[1],"Force Parallelism.");
...
void EmitGenInstrClass(FILE *output) {
  ...
  /* Emitting generic instruction class declaration */
...
void EmitFormatClasses(FILE *output) {
  ...
  /* Emitting format class declaration */
