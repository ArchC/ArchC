...
void ac_tlm_port::read(ac_ptr buf, uint32_t address, int wordsize,
                       sc_core::sc_time &time_info, unsigned int procId) {
...
void ac_tlm_port::read(ac_ptr buf, uint32_t address,
  ...
  case 8:
	...
	for (int j = 0; (i < n_words) && (j < 4); i++, j++) { 
	  (buf.ptr8)[i] = ((uint8_t*)&rsp.data8)[j];
	}
        i--;
      }
    }
  ...
  case 16:
	...
	for (int j = 0; (i < n_words) && (j < 2); i++, j++) { 
	  (buf.ptr16)[i] = ((uint16_t*)&rsp.data16)[j];
	}
        i--;
      }
    }
...
void ac_tlm_port::read(ac_ptr buf, uint32_t address, int wordsize, int n_words,
                       sc_core::sc_time &time_info, unsigned int procId) {
...
void ac_tlm_port::write(ac_ptr buf, uint32_t address,
  ...
  case 8:
	...
	((uint8_t*)&req.data8)[j] = (buf.ptr8)[i];
      }
      i--;
      (*this)->transport(req);
    }
  ...
  case 16:
	...
	((uint16_t*)&req.data16)[j] = (buf.ptr16)[i];
      }
      i--;
      (*this)->transport(req);
    }
...
void ac_tlm_port::write(ac_ptr buf, uint32_t address, int wordsize, int n_words,
                        sc_core::sc_time &time_info, unsigned int procId) {
