...
/*
  This looks for the first occurrence of 'symbol' in the mapping list.

  This function returns only the first occurrence as it's found in the list,
...
version internally. The formatting done basically removes additional whitespaces
and put an additional scape character for every one found.  The formatted asm
...
string. An argument is basically a field of the instruction where the values will
